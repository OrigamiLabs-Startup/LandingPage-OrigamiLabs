...
function getWhatsappUrl() {
  const configuredUrl = import.meta.env.VITE_WHATSAPP_URL;

  if (configuredUrl) {
    try {
      const url = new URL(configuredUrl);
      const allowedHost = url.hostname === "wa.me" || url.hostname === "api.whatsapp.com";

      if (url.protocol === "https:" && allowedHost) {
        return url.toString();
      }
    } catch {
      // Fall through to the deliberate placeholder below.
    }
  }
